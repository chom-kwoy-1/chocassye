...
async function make_db(num_data: number): Promise {
  const BATCH_SIZE = 16;

  const all_sentences: string[] = [];
  const ngram_map: Map = new Map();
  ...
  return insert_documents(insert_into_db, BATCH_SIZE, num_data).then(() => {
...
async function test() {
  ...
  console.log(
    "Overestimated by ",
    final_ids.size / matching_sids.length,
    "times",
  );
