...
export default function Histogram(props) {
  ...
  let middleKoreanHits = data
    .filter((decade) => decade.period >= 1400 && decade.period < 1600)
    .map((decade) => decade.num_hits)
    .reduce((a, b) => a + b, 0);

  // Sum number of hits from 1600
  let modernKoreanHits = data
    .filter((decade) => decade.period >= 1600)
    .map((decade) => decade.num_hits)
    .reduce((a, b) => a + b, 0);
