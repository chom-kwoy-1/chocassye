...
import CompactPrefixTree from "./prefix_tree";
...
function normalize_suffix(suffixes: Set<string>): Set<string> {
  ...
  const arr = Array.from(suffixes).map((suffix) =>
...
function transform(result: ParsedRegExp): ParsedRegExp {
  ...
  while (result.prefix.size > MAX_SET_SIZE) {
    ...
    const max_prefix_size =
      Math.max(...Array.from(result.prefix).map((prefix) => prefix.length)) - 1;
    result.prefix = new Set(
      Array.from(result.prefix).map((prefix) => {
  ...
  while (result.suffix.size > MAX_SET_SIZE) {
    ...
    const max_suffix_size =
      Math.max(...Array.from(result.suffix).map((suffix) => suffix.length)) - 1;
    result.suffix = new Set(
      Array.from(result.suffix).map((suffix) => {
...
function find_ids(
  ...
  if (match.type === "and") {
    return (match as And).matches.reduce(
      (acc: Set<number> | null, m: Match): Set<number> => {
