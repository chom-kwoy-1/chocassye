...
export function year_and_bookname_from_filename(file) {
  ...
  if (
    !path.parse(file).dir.includes("unknown") &&
    !path.parse(file).dir.includes("sktot")
  ) {
...
export function parse_year_string(year_string) {
  let ys_norm = year_string
    .replace(/\[[^\]]*\]/g, "")
    .replace(/\([^\)]*\)/g, "");

  let year, year_start, year_end;
  if (
    ys_norm.match(/^[0-9][0-9][0-9][0-9]$/) !== null ||
    ...
    year_string.match(/[0-9][0-9]세기 ?(전기|전반|전반기|초|초반|초기)/) !==
    null
  ) {
    let matched = year_string.match(
      /[0-9][0-9]세기 ?(전기|전반|전반기|초|초반|초기)/,
    )[0];
    year = parseInt(matched.slice(0, 2)) * 100 - 75;
    year_start = year - 25;
    year_end = year + 24;
  } else if (
    year_string.match(/[0-9][0-9]세기 ?(후기|후반|후반기|말|말기)/) !== null
  ) {
    let matched = year_string.match(
      /[0-9][0-9]세기 ?(후기|후반|후반기|말|말기)/,
    )[0];
