...
import { convertChineseTonesToArrows, ipaToArpabet } from "./utils";
...
export class Tokenizer {
  ...
  protected _preprocess(text: string): PreprocessResult {
    const { words, languageMap, segments } =
      this._detectLanguagesAndSegment(text);
  ...
  private _processTokens(
    ...
    const posResults = simplePOSTagger.tagWords(
      cleanWords.map(({ token }) => token),
    );
