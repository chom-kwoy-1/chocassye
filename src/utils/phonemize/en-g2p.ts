...
const dictionary: EnDict = JSON.parse(
  fs.readFileSync("chocassye-corpus/en/dict.json", "utf8"),
);
const homographs: HomographDict = JSON.parse(
  fs.readFileSync("chocassye-corpus/en/homographs.json", "utf8"),
);
...
export class G2PModel implements G2PProcessor {
  ...
  private predictInternal(
    ...
    if (decomposition && decomposition.length > 1) {
      const pronunciations = decomposition.map((part) =>
        this.wellKnown(part)?.replace(/ˈ/g, ""),
      );
  ...
  private assignStress(syllables: string[], word: string): number {
    ...
    if (syllables.length === 2) {
      ...
      if (
        ["be", "de", "re", "un", "in", "ex", "pre"].some((prefix) =>
