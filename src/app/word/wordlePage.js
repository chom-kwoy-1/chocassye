...
export function WordleImpl(props) {
  ...
  const gameName =
    todayNum !== -1 ? t("마초아쎠 #") + todayNum : t("연습 문제");
  const longGameName =
    todayNum !== -1
      ? t("오늘의 마초아쎠 #") + todayNum
      : t("마초아쎠 연습 문제");
  ...
  React.useEffect(() => {
    ...
    const storedCorrectLetters = localStorage.getItem(
      prefix + "correctLetters",
    );
    const storedMisplacedLetters = localStorage.getItem(
      prefix + "misplacedLetters",
    );
  ...
  React.useEffect(() => {
    ...
    localStorage.setItem(
      prefix + "correctLetters",
      JSON.stringify(Array.from(correctLetters)),
    );
    localStorage.setItem(
      prefix + "misplacedLetters",
      JSON.stringify(Array.from(misplacedLetters)),
    );
    localStorage.setItem(
      prefix + "wrongLetters",
      JSON.stringify(Array.from(wrongLetters)),
    );
  ...
  const inputLetter = React.useCallback(
    async (letter) => {
      if (isFinished) {
        return;
      }
  ...
  async function shareResult() {
    let resultText = hasWon
      ? `${gameName}: ${curRow}/${NUM_ROWS}🎉\n`
      : `${gameName}: X/${NUM_ROWS}😅\n`;
  ...
  return (
    <React.Fragment>
      ...
      <Dialog
        open={isFinished && dialogOpen}
        onClose={() => {
          setDialogOpen(false);
        }}
      >
        <DialogTitle>{longGameName}</DialogTitle>
        <DialogContent>
          <Box sx={{ textAlign: "center", padding: 2 }}>
            <Typography variant="h5" gutterBottom>
              {hasWon
                ? t("축하합니다! 단어를 맞추셨습니다.")
                : t("아쉽게도 단어를 맞추지 못하셨습니다.")}
            </Typography>
